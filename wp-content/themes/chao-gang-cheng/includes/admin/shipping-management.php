...
function chao_gang_cheng_apply_shipping_management_rates( $rates, $package ) {
    ...
    // 免運一目了然。
    //
    // 【2026-08 更新】原本以為超商／門市自取的原生名稱已經夠明確，不需要
    // 改名，但實際查證後發現超商取貨的原生方式標題其實是「綠界物流 超商
    // 取貨 7-ELEVEN」、門市自取則是「自行取貨」——兩者都不是理想的客顯
    // 文字（超商那筆還帶了「綠界物流」這個後端物流商名稱，不該讓客人在
    // 結帳頁／訂單通知信看到），這裡一併補上覆寫，統一改用「運費管理」
    // 頁面自己定義的方式名稱（見 chao_gang_cheng_shipping_methods_structure()
    // 的 label 欄位），確保前台顯示文字跟後台設定頁用詞一致、且不出現
    // 綠界物流字樣。
